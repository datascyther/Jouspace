...
// ── Voice chat (sendVoice) ────────────────────────────────────────────────────

describe('useJouspaceIntelligence.sendVoice', () => {
  it('inserts the transcript bubble, then streams the assistant reply', async () => {
    vi.stubEnv('DEV', true);
    localStorage.removeItem('jouspace:runtimeUrl');
    fetchMock.mockReturnValue(
      Promise.resolve(
    ...
    await waitFor(() => {
      expect(
        result.current.messages.some(
          (m) => m.role === 'user' && m.text === 'This is what I said'
        )
      ).toBe(true);
      expect(
        result.current.messages.some(
          (m) => m.role === 'assistant' && m.text === 'Here is my answer'
        )
      ).toBe(true);
    });
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: expect.stringContaining('data:audio/wav;base64,AAAA'),
      })
    );
  });

  it('ends with exactly [user transcript, assistant reply] and settles idle', async () => {
    vi.stubEnv('DEV', true);
    localStorage.removeItem('jouspace:runtimeUrl');
    fetchMock.mockReturnValue(
      Promise.resolve(
  ...
  it('includes the prior conversation in the voice request body', async () => {
    vi.stubEnv('DEV', true);
    localStorage.removeItem('jouspace:runtimeUrl');
    const postedBodies: string[] = [];
    fetchMock.mockImplementation((_url, opts) => {
      postedBodies.push((opts as { body?: string }).body ?? '');
      return Promise.resolve(
        makeSSEResponse(['{"transcript":"hi"}', '{"text":"ok"}'])
      );
    });
    ...
    act(() =>
      result.current.sendVoice({ dataUrl: 'data:audio/wav;base64,BBBB' })
    );
    await waitFor(() => expect(postedBodies.length).toBe(2));

    const voiceBody = JSON.parse(postedBodies[1]);
    expect(voiceBody.messages).toEqual([
