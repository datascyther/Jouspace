...
- Auth: **Firebase is the identity provider (Google + email/password), identity only**. Profile data (display name, joined date) stored locally in localStorage; journal stays local-first with no cloud DB. Firebase is initialized in `src/lib/firebaseClient.ts`; native uses `@capacitor-firebase/authentication` (`skipNativeAuth: false` so the native session persists and `authStateChange` fires like the web SDK), web uses the Firebase JS SDK **redirect** flow (chosen for robustness on browsers where popups are blocked). Requires `.env` `VITE_FIREBASE_*` + `android/app/google-services.json` (see `DEPLOYMENT.md` §3). "Continue without an account" is still intentional local-first UI — don't remove it.
